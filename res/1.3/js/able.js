/*
**************************************************************** 
** AngryBirdsLevelEditor(ABLE) by MlgmXyysd
** http://www.meowcat.tk/
** Copyright (C) 2013-2017 MeowCat Studio Powered by 
** MlgmXyysd All Rights Reserved.
****************************************************************
*/
...
function help() {
  var message = "欢迎来到愤怒的小鸟关卡编辑器！\n\n"
               ...
               +"注意：暂不支持Apple设备\n\n"
               +"编辑教程：\n"
               +"在你的设备中寻找到lua关卡文件并反编译此文件，"
               +"然后粘贴在编辑器的文本框中,按“加载/重载”,编辑完成之后,按“保存更改”,然后复制文本框里的内容，\n"
               +"将内容粘贴到新创建的lua关卡文件中。\n"
               +"最后,覆盖现有的lua关卡文件并保存。\n"
			   +"鸣谢：\n"
			   +"Q墙果 钟声扬001 小鸟大战绿皮猪 等支持本作品的吧友\n";
  return message;
}
...
function hide(el,offset){
		...
		setTimeout("d.getElementById('loading').style.width = '0'",2000)
		setTimeout("d.getElementById('loading').style.height = '0'",2000)
    }

function preloadImagesCheck() {
  var ilDiv = d.getElementById('loading');
  if(numberOfImagesLoaded < numberOfImages) {
    ilDiv.innerHTML = "<img src='images/loading.png' style='height:90%;' ondragstart='return false;'><h1>小喵正在玩命加载图片资源中，请稍等...("+((numberOfImagesLoaded/numberOfImages)*100 >> 1)+"%)</h1>";
    setTimeout(preloadImagesCheck,100);
  } else {
    drawToolSet();
    setInterval(drawCanvas,100);
	ilDiv.innerHTML = "<img src='images/loading.png' style='height:90%;' ondragstart='return false;'><h1>加载已完成...</h1>";
	hide(loading,0.01);
  }
}
...
function deleteObject(obj) {
  ...
  if(delete(levelData.world[obj.name]) == false) {
    alert("删除" + name + "时发生错误，请联系网站管理员(mlgmxyysd@vip.qq.com")
  } else {
